...
def hello_man():
    try:
        if request.method == 'GET':
            ...
            if repayStyle =='先息后本':
                xxhb=interest_first_then_capital(money,rate,term)
                return render_template('result.html',repayMethod=xxhb)
    except Exception:
...
def hello_vip():
    try:
        if request.method == 'GET':
            ...
            if repayStyle =='先息后本':
                xxhb=interest_first_then_capital(money,rate,term)
                return render_template('vip_result.html',repayMethod=xxhb)
    except Exception:
        return render_template('warn.html')
